; U202212021-网安2204班-戴申宇 | 2025-02-13 | 级别四
...
LABEL_GDT:
    ...
    Descriptor 0, 0ffffh, DA_DRW ; Normal 描述符
    Descriptor 0, 0fffffh, DA_CR | DA_32 | DA_LIMIT_4K ; 0 ~ 4G
    Descriptor 0, 0fffffh, DA_DRW | DA_LIMIT_4K ; 0 ~ 4G
    Descriptor 0, SegCode32Len - 1, DA_CR | DA_32 ; 非一致代码段, 32
    Descriptor 0, 0ffffh, DA_C ; 非一致代码段, 16
    Descriptor 0, DataLen - 1, DA_DRW ; Data
    Descriptor 0, TopOfStack, DA_DRWA | DA_32 ; Stack, 32 位
...
; 字符串
_szPMMessage db "U202212021 - Shenyu Dai - calvvnono@gmail.com", 0Ah, 0Ah, 0 ; 进入保护模式后显示此字符串
...
; ----------------- 任务相关定义 ---------------------------------------------
; 任务栈大小
%define STACK_SIZE 528 ; 调整栈大小

; 任务表
task_table:
    dd task_love_entry, 16, task_love_stack + STACK_SIZE
    dd task_very_entry, 10, task_very_stack + STACK_SIZE ; 优先级最高, 在栈顶存储返回地址
    dd task_hust_entry, 8, task_hust_stack + STACK_SIZE
    dd task_mrsu_entry, 6, task_mrsu_stack + STACK_SIZE ; 优先级最低

current_task dd task_love_entry ; 初始任务
